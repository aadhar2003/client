...
router.put('/updatepost/:id', fetchuser , async (req, res) => {
    const {title , description, procedure, vegetarian,  tag, imageURL, timereq, ingredients} = req.body;
    ...
    try{
        ...
        if (timereq) { newPost.timereq = timereq};
        if (ingredients) { newPost.ingredients = ingredients};
        newPost.edited = true;
